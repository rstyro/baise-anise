...
@RestController
@RequestMapping("/business/bizProductSpuAttr")
public class BizProductSpuAttrController extends BaseController {
    ...
    public R listByProduct(Long productId) {
        LambdaQueryWrapper<BizProductSpuAttr> query = new LambdaQueryWrapper<>();
        query.eq(BizProductSpuAttr::getProductId, productId);
        List<BizProductSpuAttr> attrs = bizProductSpuAttrService.list(query);

        List<Map<String, Object>> result = new ArrayList<>();
        if (!attrs.isEmpty()) {
            Map<Long, BizAttribute> attrMap = bizAttributeService.listByIds(
                    attrs.stream().map(BizProductSpuAttr::getAttrId).collect(Collectors.toList())
            ...
            Map<Long, BizAttributeValue> valueMap = bizAttributeValueService.listByIds(
                    attrs.stream().map(BizProductSpuAttr::getAttrValueId).collect(Collectors.toList())
            ).stream().collect(Collectors.toMap(BizAttributeValue::getId, a -> a));

            for (BizProductSpuAttr attr : attrs) {
                Map<String, Object> item = new HashMap<>();
                item.put("id", attr.getId());
                item.put("productId", attr.getProductId());
    ...
    public R save(@RequestBody Map<String, Object> data) {
        ...
        LambdaQueryWrapper<BizProductSpuAttr> deleteQuery = new LambdaQueryWrapper<>();
        deleteQuery.eq(BizProductSpuAttr::getProductId, productId);
        bizProductSpuAttrService.remove(deleteQuery);

        for (Map<String, Object> attr : attrs) {
            Long attrId = ((Number) attr.get("attrId")).longValue();
            Long attrValueId = ((Number) attr.get("attrValueId")).longValue();

            BizProductSpuAttr spuAttr = new BizProductSpuAttr()
                    .setProductId(productId)
                    .setAttrId(attrId)
                    .setAttrValueId(attrValueId);
            bizProductSpuAttrService.save(spuAttr);
        }
    ...
    public R del(Long id) {
        bizProductSpuAttrService.removeById(id);
        return R.ok();
    }
